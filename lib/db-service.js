...
export const dbService = {
  ...
  books: {
    ...
    create: async (book) => {
      ...
      const id = await insert(
        `INSERT INTO books (title, author, category_id, description, isbn, price, discount, availability, stock, cover_image, tags) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          book.title,
          book.author,
    ...
    update: async (id, updates) => {
      ...
      if (updates.weight !== undefined) {
        fields.push('weight = ?');
        values.push(updates.weight != null && updates.weight !== '' ? parseFloat(updates.weight) : null);
      }
