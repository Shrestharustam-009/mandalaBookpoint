...
class PaymentService {
  ...
  async getTransactionStatus(transactionId, orderNo) {
    ...
    const officeId = this.config.merchant.merchantId;

    // Build the request exactly as defined in PHP demo Inquiry.php ExecuteJose
    const inquiryData = {
      apiRequest: {
    ...
    try {
      // Use the exact transactionList endpoint which supports JOSE POST
      const endpoint = '/api/1.0/Inquiry/transactionList';
      const result = await this.makeRequest(endpoint, 'POST', inquiryData, true);

      // Decoded result matches standard JOSE format decrypted on response
      const data = result?.response || result?.Response || result;
      const list = data?.Data?.transactionList || data?.transactionList || data?.transactionListResult || [];
      const tx = list[0] || {};

      return {
        status: tx?.status || tx?.Status || tx?.respCode || 'UNKNOWN',
        transactionId: tx?.transactionId || tx?.transactionID || transactionId,
        orderNo: tx?.orderNo || orderNo,
        amount: tx?.amount || tx?.transactionAmount?.amount,
        raw: result,
      };
    } catch (error) {
